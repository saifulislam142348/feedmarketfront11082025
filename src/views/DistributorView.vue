      ...
      <table class="min-w-full table-fixed border-collapse text-sm">
        <thead class="bg-blue-600 text-white sticky top-0 z-10">
          <tr>
            ...
            <th class="border border-blue-700 px-4 py-2 w-32 text-left">Distributor</th>
        ...
        <tbody>
          <tr v-for="(row, index) in flatData" :key="index" class="hover:bg-gray-100 transition-all">
            ...
            <td class="border border-gray-300 px-4 py-2 font-medium">{{ row.code }} {{ row.name }}</td>
    ...
    for (const region of company.regions) {
      ...
      for (const area of region.areas) {
        ...
        for (const territory of area.territories) {
          if (!territory.agents) continue
          for (const agent of territory.agents) {
            if (!agent.months) continue
...
async function fetchData() {
  try {
    ...
    const url = `https://feedend.bditfirm.com/api/market/distributor?${params.toString()}`
