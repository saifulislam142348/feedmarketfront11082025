      ...
      <table class="min-w-full table-fixed border-collapse text-sm">
        <thead class="bg-blue-600 text-white sticky top-0 z-10">
          <tr>
            ...
            <th class="border border-blue-700 px-4 py-2 w-32 text-left">Region</th>
            <th class="border border-blue-700 px-4 py-2 w-32 text-left">Area</th>
            <th class="border border-blue-700 px-4 py-2 w-32 text-left">Territory</th>
            <th class="border border-blue-700 px-4 py-2 w-32 text-left">Sales Officer</th>
            <th class="border border-blue-700 px-4 py-2 w-28 text-left">Month-Year</th>
        ...
        <tbody>
          <tr v-for="(row, index) in flatData" :key="index" class="hover:bg-gray-100 transition-all">
            ...
            <td class="border border-gray-300 px-4 py-2 font-medium">{{ row.salesOfficer }}</td>
            <td class="border border-gray-300 px-4 py-2 font-medium">{{ capitalize(row.month) }} - {{ row.year }}</td>
    ...
    for (const region of company.regions) {
      ...
      for (const area of region.areas) {
        ...
        for (const territory of area.territories) {
          if (!territory.salesOfficers) continue
          for (const salesOfficer of territory.salesOfficers) {
            if (!salesOfficer.months) continue
            for (const month of salesOfficer.months) {
              rows.push({
                company_name: company.company_name,
                region: region.region,
                area: area.area,
                territory: territory.territory,
                salesOfficer: salesOfficer.name,
                month: month.month,
                year: month.year,
                qty: month.do_qty || 0,
              })
...
async function fetchData() {
  try {
    ...
    const url = `https://feedend.bditfirm.com/api/market/sales-officer?${params.toString()}`
