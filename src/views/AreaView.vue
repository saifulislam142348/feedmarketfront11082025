<template>
  <div class="p-6 mx-auto max-w-7xl">
    <h2 class="text-3xl font-bold mb-6">Market Report (Area)</h2>

    <!-- তোমার ফিল্টার কম্পোনেন্ট -->
      ...
      <table class="min-w-full table-fixed border-collapse text-sm">
        ...
        <tbody>
          <tr v-for="(row, idx) in flatData" :key="idx" class="hover:bg-gray-100 transition-all">
            <td class="border border-gray-300 px-4 py-2 font-bold text-gray-700">
              {{ row.company_name }}
            </td>
...
async function fetchData() {
  try {
    ...
    const url = `https://feedend.bditfirm.com/api/market/area?${params.toString()}`;
...
<style>
/* Optional: ফিক্সড হেডারসহ scroll ঠিক করার জন্য */
thead tr {
  position: sticky;
  top: 0;
  z-index: 10;
